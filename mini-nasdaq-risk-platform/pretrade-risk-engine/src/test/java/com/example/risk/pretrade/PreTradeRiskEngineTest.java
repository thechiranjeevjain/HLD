...
class PreTradeRiskEngineTest {
    ...
    void acceptsAndReservesExposureInMemory() {
        ...
        var decision = engine.submit(new OrderRequest("OK-1", "NASDAQ-DEMO", "MSFT", Side.BUY, 100,
                new BigDecimal("410.25"), false));

        assertThat(decision.status()).isEqualTo(OrderStatus.ACCEPTED);
        assertThat(engine.state().accounts().get("NASDAQ-DEMO").reservedNotional()).isEqualByComparingTo("41025.0000");
    }

    @Test
    void killSwitchRejectsBeforeReservation() {
        PreTradeRiskEngine engine = new PreTradeRiskEngine();
        engine.setKillSwitch(new KillSwitchRequest("ACCOUNT", "NASDAQ-DEMO", true, "test"));

        var decision = engine.submit(new OrderRequest("KILL-1", "NASDAQ-DEMO", "MSFT", Side.BUY, 100,
                new BigDecimal("410.25"), false));

        assertThat(decision.status()).isEqualTo(OrderStatus.REJECTED);
        assertThat(decision.reason()).contains("Active kill switch");
        assertThat(engine.state().accounts().get("NASDAQ-DEMO").reservedNotional()).isEqualByComparingTo("0.0000");
    }
