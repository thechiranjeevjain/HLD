...
I built a modular-monolith fraud investigation platform that combines deterministic rules with a guarded AI assistant. The AI can only call approved read tools and retrieve cited policy; it cannot access the database or execute irreversible actions. PostgreSQL stores the audit truth, the outbox feeds Kafka at least once, and a senior analyst must approve sensitive recommendations.
...
- Enforced least-privilege AI through an explicit read-tool allowlist, masked evidence, complete tool/prompt audit records, and mandatory senior approval for sensitive actions.
